...
pub fn main() {
    ...
    file.read_to_end(&mut buffer)
        .expect("Cannot create file buffer");
    let buffer: &[u8] = buffer.as_mut_slice();

    let header =
        fbxcel::low::FbxHeader::read_fbx_header(buffer).expect("Failed to load FBX header");
    ...
    match parser_version {
        pull_parser::ParserVersion::V7400 => {
            let mut parser = pull_parser::v7400::from_reader(header, buffer)
